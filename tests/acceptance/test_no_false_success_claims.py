...
class TestBrowserActionsNeverVerified:
    ...
    def test_open_url_keys_all_ok_is_accepted_unverified(self):
        ...
        r = _registry()
        with (
            patch("prometheus.execution.tools.CONFIG", {"urls": {"gmail": "https://mail.google.com",
...
class TestAppOpenActionsNeverClaimVerifiedIfUnconfirmed:
    ...
    def test_fresh_launch_with_no_known_process_is_accepted_unverified(self):
        ...
        r = ToolRegistry()
        with (
            patch("prometheus.execution.tools._APP_PROCESS_NAMES", {}),  # no process name mapping
            patch("prometheus.execution.tools.command_exists", return_value=True),
            patch("prometheus.execution.tools.ToolRegistry._launch_with_fallback",
    ...
    def test_fresh_launch_pgrep_no_match_is_accepted_unverified(self):
        ...
        with (
            patch("prometheus.execution.tools._APP_PROCESS_NAMES", {"myapp": "myapp"}),
            patch("prometheus.execution.tools.command_exists", return_value=True),
    ...
    def test_open_app_message_never_says_is_open_when_unconfirmed(self):
        from prometheus.execution.tools import ToolRegistry, ToolResult as TR
        r = ToolRegistry()
        with (
